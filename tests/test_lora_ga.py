...
class TestLoraGAInitialization:
    ...
    def test_all_directions(self, direction):
        ...
        lora_ga_config = LoraGAConfig(direction=direction)
        lora_config = LoraConfig(
            r=8,
            lora_alpha=16,
            target_modules=["0"],
    ...
    def test_all_scales(self, scale):
        ...
        lora_ga_config = LoraGAConfig(scale=scale)
        lora_config = LoraConfig(
            r=8,
            lora_alpha=16,
            target_modules=["0"],
    ...
    def test_without_preprocessing_falls_back(self):
        """Test that without preprocessing, LoRA-GA falls back to gaussian initialization."""
        model = torch.nn.Sequential(torch.nn.Linear(20, 20))
        ...
        # Should not raise - falls back to gaussian initialization
        peft_model = get_peft_model(model, lora_config)
        assert peft_model is not None
