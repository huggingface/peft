...
class AdaptionPromptTester(TestCase, PeftCommonTester):
    ...
    def test_save_pretrained_regression(self) -> None:
        ...
        if self.mistral_available:
            ...
            with tempfile.TemporaryDirectory() as tmp_dirname:
                ...
                # check if `model.safetensors` is not present
                self.assertFalse(os.path.exists(os.path.join(tmp_dirname, "mode.safetensors")))
    ...
    def test_sequence_adapter_ops(self) -> None:
        ...
        if self.mistral_available:
            ...
            # Add new adapter 1.
            adapted_mistral.add_adapter("adapter 1", AdaptionPromptConfig(adapter_layers=3, adapter_len=8, task_type="CAUSAL_LM"))
            # Test zero-init
