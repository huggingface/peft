...
class TestRequiresGrad:
    ...
    def test_loading_model_requires_grad_set_correctly(self, config_cls, is_trainable, tmp_path):
        ...
        # load a third adapter with the opposite trainability
        model.load_adapter(tmp_path, adapter_name="opposite", is_trainable=not is_trainable)
        trainable_adapter = "default" if is_trainable else "opposite"
        for name, param in model.named_parameters():
            if skip_ranknum and "ranknum" in name:
                continue
            if f".{trainable_adapter}." in name or name.endswith(f".{trainable_adapter}"):
                assert param.requires_grad
            else:
                assert not param.requires_grad
