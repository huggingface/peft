...
class TestTorchCompileCausalLM:
    ...
    def test_causal_lm_add_weighted_adapter_lora_adapter_compile(self, tokenizer, data):
        ...
        # TODO: more tests, including tests that are expected to fail

    # @pytest.mark.single_gpu_tests
    # @pytest.mark.parametrize("torch_compile", [True, False])
    # def test_foobar_reproducible(self, torch_compile, tokenizer, data, tmp_path):
    #     r"""No PEFT, no compile, just pure transformers"""
    #     tmp_dir = tmp_path / "model"
    ...
    #         "seed": 0,
    #     }
    #     training_args = TrainingArguments(torch_compile=False, **train_kwargs)
    #     trainer = Trainer(
    #         model=model,
    #         train_dataset=data["train"],
    #         args=training_args,
    ...
    #     torch.manual_seed(0)
    #     trainer.train()
    #     total_loss_no_compile = trainer.state.log_history[-1]["train_loss"]

    #     model.eval()
    #     atol, rtol = 1e-4, 1e-4
    #     with torch.inference_mode():
    #         output_no_compile = model(sample)
    #     assert torch.isfinite(output_no_compile.logits).all()
    #     # sanity check: model was updated
    #     assert not torch.allclose(output_before.logits, output_no_compile.logits, atol=atol, rtol=rtol)

    #     # now train again, possibly with compile
    #     torch.manual_seed(0)
    #     model = AutoModelForCausalLM.from_pretrained(
    #         self.model_id,
    #         device_map="auto",
    #     )
    #     training_args = TrainingArguments(torch_compile=torch_compile, **train_kwargs)
    #     trainer = Trainer(
    #         model=model,
    #         train_dataset=data["train"],
    #         args=training_args,
    ...
    #     torch.manual_seed(0)
    #     trainer.train()
    #     total_loss_compile = trainer.state.log_history[-1]["train_loss"]

    #     model.eval()
    #     with torch.inference_mode():
    #         output_compile = model(sample)
    #     assert torch.isfinite(output_compile.logits).all()
    #     assert torch.allclose(output_no_compile.logits, output_compile.logits, atol=atol, rtol=rtol)
    #     assert total_loss_no_compile == total_loss_compile
