...
class TestQuantization:
    ...
    def test_bnb_8bit_merge_bias_applies_scaling(self):
        ...
        # unlike the base path (`lora/layer.py`). CPU-runnable: int8 dequant needs CUDA, so
        # only the weight dequant is stubbed — the bias formula under test is untouched.
        # scaling 8.0 (r=4, lora_alpha=32) makes a missing factor show up as max-abs ~3.35.
        ...
        try:
            ...
            model = get_peft_model(
                BnbNet(),
                LoraConfig(r=4, lora_alpha=32, target_modules=["lin"], lora_bias=True, init_lora_weights=False),
            )
            ...
            # safe_merge NaN guard must raise ValueError (the missing `.all()` raised an
            # ambiguous-Boolean RuntimeError instead on multi-element bias).
    ...
    @pytest.mark.skipif(not is_bnb_4bit_available(), reason="bitsandbytes 4-bit not available")
    def test_bnb_4bit_merge_bias_applies_scaling(self):
        # Same as above for the 4-bit path (#3741 comment scope). Params4bit storage needs
        # CUDA to dequantize, so the stub returns zeros of the right shape — the weight path
        # result is irrelevant here, only the bias formula is asserted.
        import bitsandbytes as bnb
        ...
        orig_dequant = bnb_lora.dequantize_bnb_weight
        bnb_lora.dequantize_bnb_weight = lambda w, state=None: torch.zeros(16, 16)
        try:

            class Bnb4Net(torch.nn.Module):
                def __init__(self):
                    super().__init__()
                    self.lin = bnb.nn.Linear4bit(16, 16, bias=True)
