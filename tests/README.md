...
- Most PEFT methods like LoRA are initialzed in a way that they don't have an effect before training them. This can be a hindrance for testing, as many tests require the adapter to affect the output to work properly. Therefore, most PEFT methods offer a config option `init_weights=False` which initializes their weights to have a non-zero effect. The name of the argument is not fully consistent, e.g. for LoRA we need to pass `init_lora_weights=False`. There is a helper function, `set_init_weights_false`, which updates the `kwargs` accordingly.
