...
class TestShira:
    ...
    def test_mlp_single_adapter_merge_unmerge(self, mlp):
        ...
        assert torch.all(peft_model.base_model.model.lin1.shira_weight['default'] == 0)
        assert torch.all(peft_model.base_model.model.lin2.shira_weight['default'] == 0)

        shira_assign_val1 = torch.randn_like(peft_model.base_model.model.lin1.shira_weight['default'])
        peft_model.base_model.model.lin1.shira_weight['default'] = shira_assign_val1
        shira_assign_val2 = torch.randn_like(peft_model.base_model.model.lin2.shira_weight['default'])
        peft_model.base_model.model.lin2.shira_weight['default'] = shira_assign_val2

        manual_merge_weight1 = peft_model.base_model.model.lin1.base_layer.weight + peft_model.base_model.model.lin1.get_delta_weight('default')
        manual_merge_weight2 = peft_model.base_model.model.lin2.base_layer.weight + peft_model.base_model.model.lin2.get_delta_weight('default')

        peft_model.base_model.model.lin1.merge(adapter_names=['default'])
        peft_model.base_model.model.lin2.merge(adapter_names=['default'])

        assert torch.all(manual_merge_weight1 - peft_model.base_model.model.lin1.base_layer.weight.data == 0)
        assert torch.all(manual_merge_weight2 - peft_model.base_model.model.lin2.base_layer.weight.data == 0)
    ...
    def test_mlp_multiple_adapters_same_attachment_points(self, mlp):
        ...
        # creates a default SHiRA adapter
        peft_model = get_peft_model(mlp, config)
        config2 = ShiraConfig(r=2, target_modules=["lin1", "lin2"])
    ...
    def test_mlp_multiple_adapters_different_attachment_points(self, mlp):
        ...
        # creates a default SHiRA adapter
        peft_model = get_peft_model(mlp, config)
        config2 = ShiraConfig(r=2, target_modules=["lin1", "lin2", "lin3"], random_seed=52)
        peft_model.add_adapter("other", config2)

        assert len(peft_model.base_model.model.lin1.shira_weight.keys()) == 2
        assert len(peft_model.base_model.model.lin2.shira_weight.keys()) == 2
        assert len(peft_model.base_model.model.lin3.shira_weight.keys()) == 1

        return peft_model

    def test_mlp_multiple_adapters_different_attachment_points_merge_and_unload(self, mlp):
        # torch.manual_seed(0)
        ...
        peft_model = get_peft_model(mlp, config)
        config2 = ShiraConfig(r=2, target_modules=["lin1", "lin2", "lin3"], random_seed=52)
        peft_model.add_adapter("other1", config2)
        config3 = ShiraConfig(r=3, target_modules=["lin1"], random_seed=52)
        peft_model.add_adapter("other2", config3)

        assert torch.all(peft_model.base_model.model.lin1.shira_weight['default'] == 0)
        assert torch.all(peft_model.base_model.model.lin2.shira_weight['default'] == 0)
        assert torch.all(peft_model.base_model.model.lin1.shira_weight['other1'] == 0)
        assert torch.all(peft_model.base_model.model.lin2.shira_weight['other1'] == 0)
        assert torch.all(peft_model.base_model.model.lin3.shira_weight['other1'] == 0)
        assert torch.all(peft_model.base_model.model.lin1.shira_weight['other2'] == 0)

        shira_assign_val1_d = torch.randn_like(peft_model.base_model.model.lin1.shira_weight['default'])
        peft_model.base_model.model.lin1.shira_weight['default'] = shira_assign_val1_d
        shira_assign_val2_d = torch.randn_like(peft_model.base_model.model.lin2.shira_weight['default'])
        peft_model.base_model.model.lin2.shira_weight['default'] = shira_assign_val2_d

        shira_assign_val1_o1 = torch.randn_like(peft_model.base_model.model.lin1.shira_weight['other1'])
        peft_model.base_model.model.lin1.shira_weight['other1'] = shira_assign_val1_o1
        shira_assign_val2_o1 = torch.randn_like(peft_model.base_model.model.lin2.shira_weight['other1'])
        peft_model.base_model.model.lin2.shira_weight['other1'] = shira_assign_val2_o1
        shira_assign_val3_o1 = torch.randn_like(peft_model.base_model.model.lin3.shira_weight['other1'])
        peft_model.base_model.model.lin3.shira_weight['other1'] = shira_assign_val3_o1

        shira_assign_val1_o2 = torch.randn_like(peft_model.base_model.model.lin1.shira_weight['other2'])
        peft_model.base_model.model.lin1.shira_weight['other2'] = shira_assign_val1_o2

        peft_model1 = copy.deepcopy(peft_model)
        peft_model2 = copy.deepcopy(peft_model)
        peft_model3 = copy.deepcopy(peft_model)
        peft_model4 = copy.deepcopy(peft_model)

        #merge only default and other2 and unload everything...
        manual_merge_weight1 = peft_model1.base_model.model.lin1.base_layer.weight + peft_model1.base_model.model.lin1.get_delta_weight('default') + peft_model1.base_model.model.lin1.get_delta_weight('other2')
        manual_merge_weight2 = peft_model1.base_model.model.lin2.base_layer.weight + peft_model1.base_model.model.lin2.get_delta_weight('default')
        manual_merge_weight3 = peft_model1.base_model.model.lin3.base_layer.weight
        peft_model1.merge_and_unload(adapter_names=['default', 'other2'])
        assert torch.all(peft_model1.base_model.model.lin1.weight - manual_merge_weight1 == 0)
        assert torch.all(peft_model1.base_model.model.lin2.weight - manual_merge_weight2 == 0)
        assert torch.all(peft_model1.base_model.model.lin3.weight - manual_merge_weight3 == 0)

        #merge only default and other1 and unload everything...
        manual_merge_weight1 = peft_model2.base_model.model.lin1.base_layer.weight + peft_model2.base_model.model.lin1.get_delta_weight('default') + peft_model2.base_model.model.lin1.get_delta_weight('other1')
        manual_merge_weight2 = peft_model2.base_model.model.lin2.base_layer.weight + peft_model2.base_model.model.lin2.get_delta_weight('default') + peft_model2.base_model.model.lin2.get_delta_weight('other1')
        manual_merge_weight3 = peft_model2.base_model.model.lin3.base_layer.weight + peft_model2.base_model.model.lin3.get_delta_weight('other1')
        peft_model2.merge_and_unload(adapter_names=['default', 'other1'])
        assert torch.all(peft_model2.base_model.model.lin1.weight - manual_merge_weight1 == 0)
        assert torch.all(peft_model2.base_model.model.lin2.weight - manual_merge_weight2 == 0)
        assert torch.all(peft_model2.base_model.model.lin3.weight - manual_merge_weight3 == 0)

        #merge only other1 and other2 and unload everything...
        manual_merge_weight1 = peft_model3.base_model.model.lin1.base_layer.weight + peft_model3.base_model.model.lin1.get_delta_weight('other1') + peft_model3.base_model.model.lin1.get_delta_weight('other2')
        manual_merge_weight2 = peft_model3.base_model.model.lin2.base_layer.weight + peft_model3.base_model.model.lin2.get_delta_weight('other1')
        manual_merge_weight3 = peft_model3.base_model.model.lin3.base_layer.weight + peft_model3.base_model.model.lin3.get_delta_weight('other1')
        peft_model3.merge_and_unload(adapter_names=['other1', 'other2'])
        assert torch.all(peft_model3.base_model.model.lin1.weight - manual_merge_weight1 == 0)
        assert torch.all(peft_model3.base_model.model.lin2.weight - manual_merge_weight2 == 0)
        assert torch.all(peft_model3.base_model.model.lin3.weight - manual_merge_weight3 == 0)

        #merge all adapters: default, other1, and other2 and unload everything...
        manual_merge_weight1 = peft_model4.base_model.model.lin1.base_layer.weight + peft_model4.base_model.model.lin1.get_delta_weight('default') + peft_model4.base_model.model.lin1.get_delta_weight('other1') + peft_model4.base_model.model.lin1.get_delta_weight('other2')
        manual_merge_weight2 = peft_model4.base_model.model.lin2.base_layer.weight + peft_model4.base_model.model.lin2.get_delta_weight('default') + peft_model4.base_model.model.lin2.get_delta_weight('other1')
        manual_merge_weight3 = peft_model4.base_model.model.lin3.base_layer.weight + peft_model4.base_model.model.lin3.get_delta_weight('other1')
        peft_model4.merge_and_unload(adapter_names=['default', 'other1', 'other2'])
        assert torch.all(peft_model4.base_model.model.lin1.weight - manual_merge_weight1 == 0)
        assert torch.all(peft_model4.base_model.model.lin2.weight - manual_merge_weight2 == 0)
...
if __name__ == '__main__':
    test = TestShira()
    mlp = test.mlp()
    test.test_multiple_adapters_save_load(mlp, tmp_path="./")
