...
experiment_configs = {
    'sst2': dict(batch_size=256, num_epochs=60, vera_lr=4e-3, head_lr=4e-4, max_length=128, d_initial=0.1, r=512, dropout=0.0),
    'mrpc': dict(batch_size=256, num_epochs=30, vera_lr=2e-2, head_lr=1e-2, max_length=128, d_initial=0.1, r=512, dropout=0.0),
    'cola': dict(batch_size=256, num_epochs=80, vera_lr=2e-2, head_lr=1e-2, max_length=128, d_initial=0.1, r=512, dropout=0.0),
    'qnli': dict(batch_size=256, num_epochs=25, vera_lr=2e-3, head_lr=4e-3, max_length=128, d_initial=0.1, r=512, dropout=0.0),
    'rte': dict(batch_size=256, num_epochs=80, vera_lr=2e-2, head_lr=4e-3, max_length=128, d_initial=0.1, r=512, dropout=0.0),
    'stsb': dict(batch_size=256, num_epochs=40, vera_lr=2e-2, head_lr=1e-2, max_length=128, d_initial=0.1, r=512, dropout=0.0),
}
...
for key in [1234, 123456789, 1234567890, 0xffff, 0, 1, 666]:
    best_accuracy = 0.0
    peft_config = VeraConfig(task_type="SEQ_CLS", inference_mode=False, r=r, vera_dropout=dropout, projection_prng_key=key, d_initial=d_initial)

    if task in ['stsb']:
        model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, return_dict=True, max_length=max_length, num_labels=1)
    else:
        model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, return_dict=True, max_length=max_length)
