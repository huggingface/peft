...
By using LoRA from 🤗 PEFT, we can reduce the number of trainable parameters in the model to only 14% of the original trainable parameters.
...
Finally, combine everything in the two functions that you'll use to transform training and validation data. The two functions 
...
Including a metric during training is helpful for evaluating your model's performance. You can quickly load a evaluation 
method with the [🤗 Evaluate](https://huggingface.co/docs/evaluate/index) library. For this task, load 
...
Choose the base model checkpoint. For this example, we use the [SegFormer B0 variant](https://huggingface.co/nvidia/mit-b0). 
We pass the `label2id` and `id2label` dictionaries to let the `AutoModelForSemanticSegmentation` class know that we're 
interested in a custom base model where the decoder head should be randomly initialized with regards to our custom dataset. 
Note, however, that the rest of the model parameters are pre-trained and will be finetuned in a regular transfer learning setup.
...
```
At this point you can notice that in the base model (aka `model`) all 100% parameters are trainable.

## Wrap the base model as a PeftModel for LoRA training

To leverage LoRa technique, you need to wrap the base model as a `PeftModel`.  This involves two steps:

1. Define LoRa configuration with `LoraConfig`
2. Wrap the original `model` with `get_peft_model()` using the config defined in the step above.
...
we should expect to notice a lower number of trainable parameters from the `lora_model` in comparison to the original `model` 
which is indeed the case here.

You can also manually verify what modules are actually trainable in the `lora_model`.
...
Start by defining your training hyperparameters in `TrainingArguments`. You can change the values however you prefer. 
This example is meant to walk you through the workflow when using PEFT for semantic segmentation. We didn't 
perform extensive hyperparameter tuning to achieve optimal results.

Make sure to set `remove_unused_columns=False`, otherwise the image column will be dropped, and it's required here.
...
Let's now prepare our `inference_model` and run inference.
