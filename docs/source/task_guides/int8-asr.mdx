...
Quantization reduces the precision of floating point data types, reducing the memory required to store model weights. However, quantization degrades inference performance because you're losing information when you reduce the precision. 8-bit or `int8` quantization uses only a quarter precision, but it does not degrade performance because it doesn't just drop the bits or data. Instead, `int8` quantization *rounds* from one data type to another.
...
Initialize a [`~datasets.DatasetDict`] structure, and loading the [`train`] (you can load both the `train` and `validation` split) and [`test`] splits from the dataset into it:
...
Let's prepare the dataset for training. Load a feature extractor, tokenizer, and processor to process the inputs:
...
If you look at the `sampling_rate`, you'll see the audio was sampled at 48kHz. The Whisper model was pretrained on audio inputs at 16kHZ which means you'll need to downsample the audio inputs to match what the model was pretrained on. You can downsample the audio by using the [`~datasets.Dataset.cast_column`] method on the `audio` column, and setting the `sampling_rate` to 16kHz. The audio input is resampled on the fly the next time you call it:
...
Once you've cleaned up the dataset, you can now write a function to generate the correct model inputs. The function should:
...
Now that the dataset is ready, you can turn your attention to the model. Start by loading the pretrained [`openai/whisper-large-v2`]() model from [`~transformers.AutoModelForSpeechSeq2Seq`], and make sure to set the [`~transformers.BitsAndBytesConfig.load_in_8bit`] argument to `True` to enable `int8` quantization. The `device_map=auto` argument determines how to automatically load and store the model weights for you:
...
To get the model ready for `int8` quantization, use the [`prepare_model_for_int8_training`](https://github.com/huggingface/peft/blob/34027fe813756897767b9a6f19ae7f1c4c7b418c/src/peft/utils/other.py#L35) function to freeze the base model's layers, and cast the `LayerNorm` to full precision (`fp32`) for stability:
...
Pass the `TrainingArguments`, model, datasets, data collator, tokenizer, and callback to the [`~transformers.Seq2SeqTrainer`]. You can optionally set `model.config.use_cache = False` to silence any warnings. Once everything is ready, call [`~transformers.Trainer.train`] to start training!
...
Write a loop to evaluate the model performance. Set the model to evaluation mode first, and then write the loop with [`torch.cuda.amp.autocast()`](https://pytorch.org/docs/stable/amp.html) because `int8` training requires autocasting. Then, pass a batch of examples to the model to evaluate. Get the decoded predictions and labels, and then add them as a batch to the WER metric before calling `compute` to get the final WER score:
...
Load an audio sample (download the audio sample to listen to it) to transcribe, and the [`~transformers.AutomaticSpeechRecognitionPipeline`]:
...
text
"त्या लेच तर्निप्स कर्ट्स ब्रुज्ट पोटेटोस आणि फॅट मटण पीसेच तो ब्हुडल्ड आहे थिक पेपर्ट फ्लॉर फॅटन सॉस."
```

Feel free to translate the text into English to see if it aligns with what you heard!