...
For very large pretrained models, it can be challenging to finetune them for downstream tasks. Instead, you can use *prompts* to steer the model towards a particular downstream task. Typically, these prompts are handcrafted which is impractical because it relies on large validation sets. P-tuning is a method for automatically searching for prompts in a continuous space, and it is optimizable so you can find better prompts.
...
This guide will show you how to train a [`roberta-large`](https://huggingface.co/roberta-large) model with p-tuning on the `mrpc` configuration of the [GLUE](https://huggingface.co/datasets/glue) benchmark.
...
To get started, define the model, GLUE dataset, and some basic training hyperparameters:
...
Next, load the `mrpc` configuration - a corpus of sentenced pairs labeled according to whether they're semantically equivalent or not - the [GLUE](https://huggingface.co/datasets/glue) benchmark:
...
Then, load the metric for evaluating the model's performance. For this specific task, the evaluation module returns the accuracy and F1 scores.
...
Let's create the [`PeftModel`] now. P-tuning uses a prompt encoder to optimize the prompt parameters, so you'll need to initialize the [`PromptEncoderConfig`] and define the task type:
...
Upload the model with the push_to_hub function to push your model to a specifc model repository on the Hub:
...
Get some text to try out the model on, and tokenize it:
