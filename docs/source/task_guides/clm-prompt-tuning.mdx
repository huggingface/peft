...
Start by defining the model and tokenizer, the dataset and the dataset columns to train on, some hyperparameters, and the [`PromptTuningConfig`]. The [`PromptTuningConfig`] contains information about the task type, the text to initialize the prompt embedding, and the tokenizer to use:
