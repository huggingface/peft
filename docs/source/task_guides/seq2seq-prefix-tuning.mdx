...
Prefix tuning is an additive method where only a sequence of continuous task-specific vectors are attached to the beginning of to the input, or *prefix*. Only the prefix parameters are optimized, and they're added to the hidden states in every layer of the model. The tokens of the input sequence are still able to attend to the prefix as *virtual tokens*. As a result, prefix tuning stores 1000x fewer parameters than a fully finetuned model which means you can use one large language model for many tasks.
...
Use the [`~datasets.Dataset.map`] function to apply the `preprocess_function` to the dataset. You can remove the unprocessed column names since the model doesn't need them anymore:
...
Now you can setup your model and make sure it is ready for training. Specify the task in [`PrefixTuningConfig`], create the base `t5-large` model from [`~transformers.AutoModelForSeq2SeqLM`], and then wrap the model and configuration in a [`PeftModel`]. Feel free to print the model's parameters to see how many parameters you're actually training compared to fully training the model!
...
Upload the model with the [`~transformers.PreTrainedModel.push_to_hub`] function to push your model to a specifc model repository on the Hub:
...
Get some text about financial news and tokenize it:
...
Put the model on a GPU and *generate* the text sentiment:
