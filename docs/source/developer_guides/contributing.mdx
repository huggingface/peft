...
Regardless of the type of contribution (unless it’s only about the docs), you should run tests and code quality checks to ensure that your contribution doesn’t break anything and follows the standards of the project.

We provide a Makefile to facilitate those steps. Run

```sh
make test
```

for the unit tests and either one of
...
Changes to the code should be implemented in a backwards compatible way, i.e. existing code should continue to work the same way after the feature is merged.
