...
Some fine-tuning techniques such as prompt tuning are specific to language models. That means that in PEFT, it is
assumed that a 🤗 transformers is being used. Other fine-tuning techniques, however, are not restricted to specific
model types. One such technique is [LoRA](https://arxiv.org/abs/2106.09685). In this guide, we will see how LoRA can be
applied to non 🤗 transformers models.
...
_Note: For this toy example, we choose an exceedingly large number of hidden units to highlight the efficiency gains
thanks to PEFT, but those gains are in line with more realistic examples._
...
To achieve that, we first have to determine the names of the layers. The small snippet below prints them:
...
### Example 2: A computer vision model from `timm`
...
`"stages.0.blocks.0.mlp.fc2"`. How can we match those layer names specifically? Thankfully, PEFT allows us to match them
using [regular expressions](https://docs.python.org/3/library/re.html). For our case, the regex
