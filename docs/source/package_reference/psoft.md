...
The principal subspace $AB$ is constructed from the top-$r$ singular components:
...
$W_{\text{ps-tuned}} = A  diag(\alpha) R  diag(\beta) B + W_{\text{res}}.$ (PSOFT-RO, PSOFT with relaxed orthogonality)

During training, $A$, $B$, and $W_{\text{res}}$ are frozen, and only $R$ (or $R$ with $\alpha$ and $\beta$) is trainable.

For compatibility with the PEFT framework, we implement PSOFT in an additive form:
 
$W_{\text{ps-tuned}} =  W_{\text{pre}} + A(R-I)B$.
...
cfg = PSOFTConfig(
    ...
    target_modules=["c_attn", "c_proj"],    # target attention projection layers
    ...
    cayley_neumann_eps=None,                # improve convergence and numerical stability
)
...
3. **Learning Rate**: Use standard learning rates (e.g., $1\mathrm{e}{-4}$ to $5\mathrm{e}{-3}$) for stable training.
...
5. **Cayley–Neumann Approximation**: When the rank is large, enabling the Cayley–Neumann approximation can significantly improve computational efficiency, while the benefit is less pronounced for small ranks. In practice, a small number of Neumann series terms (e.g., $3$–$5$) usually provides a good balance between accuracy and efficiency.
