...
- `use_cayley_neumann`: specifies whether to use the Cayley-Neumann parameterization or use the vanilla Cayley parametrization which includes matrix inversion. Should be computationally more efficient for larger `oft_block_size` but comes at costs of bigger approximation error for orthogonality. Default is `False`.
