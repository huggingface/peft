...
def preprocess_astra(
    ...
    Args:
        ...
        run_model (`Optional[Callable[[], None]]`):
            Callback to run the model when building covariance. Typically you should run model inference on your
            sample dataset in this callback. Experiments have shown that when token count per sample is 2048, hidden
            dimension is 4096, collecting 256 distinct samples is enough. If you collect too few or too repetitive
            samples, the covariance matrix may be low-ranked and unstabilize preprocessing. You can estimate sample
            count as `HIDDEN_DIM / TOKEN_PER_SAMPLE * 128`. `run_model` can be `None` only if covariance file in
...
def crop_astra_eigens(model: nn.Module, config: LoraConfig):
    for name, module in target_modules(model, config):
        ...
        # `torch.linalg.eigh` returns eigenvalues in ascending order, so the last `rank` columns of `V` are the tail
        # eigenvectors, i.e. the ones associated with the largest eigenvalues (cf. Q[:, -r:] in the paper).
