...
class LoraGAModel(LoraModel):
    """
    LoRA model with Gradient Approximation initialization.

    This extends LoraModel to support gradient-based initialization as described
    in LoRA-GA: Low-Rank Adaptation with Gradient Approximation.

    Reference: https://arxiv.org/abs/2407.05000

    The key difference from standard LoraModel is that gradients are passed during
    initialization and used for SVD-based adapter weight initialization instead of
    random initialization.
    """

    def __init__(self, model, config, adapter_name, **kwargs):
        self.named_grad = getattr(model, "named_grad", None)
    ...
    def _create_and_replace(
        ...
        *,
        ...
        """
        Override to inject gradients into target module before layer creation.

        If not using lora_ga initialization or no gradients available, delegates
        to parent implementation.
        """
        # If using LoRA-GA initialization and gradients are available, temporarily attach them to the target
        if lora_config.init_lora_weights == "lora_ga" and self.named_grad is not None:
            grad = self.named_grad.get(current_key, None)
            # Attach gradient and config to target module temporarily
            target._loraga_temp_grad = grad
            target._loraga_temp_config = self.peft_config[adapter_name]

        try:
            # Call parent to create the layer
            result = super()._create_and_replace(
                lora_config, adapter_name, target, target_name, parent, current_key, parameter_name=parameter_name
            )
        finally:
            # Clean up temporary attributes
            if hasattr(target, '_loraga_temp_grad'):
                delattr(target, '_loraga_temp_grad')
            if hasattr(target, '_loraga_temp_config'):
                delattr(target, '_loraga_temp_config')
