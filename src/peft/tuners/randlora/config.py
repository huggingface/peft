...
class RandLoraConfig(PeftConfig):
    ...
    Args:
        ...
        projection_prng_key (`int`):
            ...
            checkpoint that did not include these projections. Defaults to `int(math.exp(1)*3.1415*1000)`.
        save_projection (`bool`):
            Whether to save the global basis_A / basis_B random basis in the state dict alongside per layer lambda /
            gamma diagonal matrices. weights. This will increase the size of the checkpoint, but guarantee that we can
