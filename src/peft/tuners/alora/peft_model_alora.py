...
from .layer import Linear, aLoraLayer #Conv2d, Conv3d, Embedding, Linear, aLoraLayer
...
#        from .mapping import MODEL_TYPE_TO_PEFT_MODEL_MAPPING, PEFT_TYPE_TO_CONFIG_MAPPING
        MODEL_TYPE_TO_PEFT_MODEL_MAPPING: dict[str, type[PeftModel]] = {
            #"SEQ_CLS": PeftModelForSequenceClassification,
            #"SEQ_2_SEQ_LM": PeftModelForSeq2SeqLM,
            "CAUSAL_LM": aLoRAPeftModelForCausalLM,
            #"TOKEN_CLS": PeftModelForTokenClassification,
            #"QUESTION_ANS": PeftModelForQuestionAnswering,
            #"FEATURE_EXTRACTION": PeftModelForFeatureExtraction,
        }
...
#        self.disable_adapters = False #NEW
        # load the config
        if config is None:
        ...
        model.ks = ks
        model.response_token_ids = response_token_ids
        model.disable_adapters = False #NEW
        return model
  ...
  #      print(args)
   #     print(kwargs)
    ...
    def get_base_model(self) -> torch.nn.Module:
        ...
        return (
            self.base_model
            if (False) #(self.active_peft_config.is_prompt_learning or self.peft_type == PeftType.POLY)
...
class aLoRAPeftModelForCausalLM(PeftModelALoRA):
    ...
    def __init__(
        self, model: torch.nn.Module, peft_config: PeftConfig, adapter_name: str = "default",alora_offsets =None,response_token_ids=None, **kwargs
    ) -> None:
        ...
        if self.disable_adapters == True:
            ...
            for i in range(len(input_ids)):
                ...
                for ii in range(len(self.response_token_ids)):
                    for idx in (torch.where(input_ids[i] == self.response_token_ids[ii][0])[0]).tolist():
                        # `response_token_ids` is `'### Response:\n'`, here we are just making sure that the token IDs match
                        if (
                            self.response_token_ids[ii].tolist()
                                == input_ids[i][idx : idx + len(self.response_token_ids[ii])].tolist()
                            ):
                            if response_token_ids_start_idx is None or idx > response_token_ids_start_idx:
...
#                    print(self.response_token_ids)
 #                   print(input_ids[i])
        ...
        #    alora_offsets = self.alora_offsets
            ...
            alora_offsets = [-1] #Do not use adapter. This does need to be consistent from train to test though.
...
#            if peft_config.peft_type == PeftType.POLY:
 #               kwargs["task_ids"] = task_ids
    ...
    def generate(self, *args, **kwargs):
#        print(f"adaps: {self.active_adapters}")
        peft_config = self.active_peft_config
