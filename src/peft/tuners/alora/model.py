...
def _adapter_names_pre_forward_hook(target, args, kwargs, adapter_names,alora_offsets):
...
class aLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: aLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: aLoraConfig, model: nn.Module):
        r"""
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for module in self.modules():
            if isinstance(module, aLoraLayer) or isinstance(module, ModulesToSaveWrapper):
                pre_forward = partial(_adapter_names_pre_forward_hook, adapter_names=adapter_names, alora_offsets = alora_offsets)
                handle = module.register_forward_pre_hook(pre_forward, with_kwargs=True)
                hook_handles.append(handle)
