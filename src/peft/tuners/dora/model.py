...
from .layer import Embedding, Linear, DoraLayer
from .config import DoraConfig


class DoraModel(LoraModel):
    """
    Creates DoRA: Weight-Decomposed Low-Rank Adaptation model from a pretrained model. The method is partially described in
    ...
    **Attributes**:
    ...
    prefix: str = "hada_"
    layers_mapping: Dict[Type[torch.nn.Module], Type[DoraLayer]] = {
    ...
    def _create_and_replace(
        ...
        config: DoraConfig,
        adapter_name: str,
        target: Union[DoraLayer, nn.Module],
        target_name: str,
        parent: nn.Module,
        current_key: str,
    ) -> None:
        ...
        if isinstance(target, DoraLayer):
