...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    """
    def __init__(self, config, model, wbits=4):
        super().__init__()
        self.peft_config = config
        self.model = model
        self._find_and_replace(wbits)
        mark_only_lora_as_trainable(self.model, self.peft_config.bias)
        self.forward = self.model.forward

    def _find_and_replace(self, wbits):
        ...
        for key, _ in self.model.named_modules():
            ...
            if target_module_found:
                ...
                new_module = MergedQuantLinear(target.qweight.shape[0]*(32//wbits), target.bias.shape[0], wbits, **kwargs)
...
class MergedQuantLinear(QuantLinear, LoraLayer):
    ...
    def __init__(self, in_features: int, out_features: int, wbits = 4, r: int = 0, lora_alpha: int = 1, lora_dropout: float = 0.0, enable_lora: List[bool] = [False], **kwargs):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0:
            ...
            output = self.lora_B(self.lora_A(drp)) * self.scaling
            result += output
        return result
