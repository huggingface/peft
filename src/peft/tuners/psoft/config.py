...
class PSOFTConfig(PeftConfig):
    ...
    Args:
        ...
        psoft_alpha (`int`): It controls PSOFT scaling factor. Same semantics as LoRA alpha.
        psoft_dropout (`float`): Dropout for PSOFT path. Same semantics as LoRA dropout.
        fan_in_fan_out (`bool`):
        ...
        psoft_svd_lowrank_niter (`int`): 
            Number of power iterations used by torch.svd_lowrank when psoft_svd='lowrank'.
        ...
        psoft_orth (`bool`):
            ...
            among column of the pre-trained weight vectors. If False, R is a free matrix without orthogonality constraints.
        psoft_mag_b (`bool`):
        ...
        num_cayley_neumann_terms (`int`): 
            ...
            Only used when use_cayley_neumann=True.
        cayley_neumann_eps (`float`):
            Optional Frobenius-norm bound for the generator matrix Q in the Cayley-Neumann approximation.
            Only used when use_cayley_neumann=True. If None (default), no rescaling is applied. 
    ...
    psoft_alpha: int = field(default=8, metadata={"help": "It controls PSOFT scaling factor. Same semantics as LoRA alpha. "})
    ...
    def __post_init__(self):
        ...
        if self.init_psoft_weights not in allowed_inits:
            raise ValueError(
                f"`init_psoft_weights` must be one of {sorted(allowed_inits)}; got {self.init_psoft_weights!r}."
            )
