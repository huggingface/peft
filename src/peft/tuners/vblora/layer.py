...
class Linear(nn.Linear, VBLoRALayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        A = Linear._get_low_rank_matrix(vblora_logits_A, vblora_vector_bank, self.topk[adapter]).reshape(
            vblora_logits_A.shape[0], -1
        )
        # B: (out_tile, rank, vector_length) -> (out_tile, vector_length, rank) -> (out_tile x vector_length, rank)
        B = (
            Linear._get_low_rank_matrix(vblora_logits_B, vblora_vector_bank, self.topk[adapter])
            .transpose(1, 2)
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                vblora_vector_bank = self.vblora_vector_bank[active_adapter].to(vblora_logits_A.device)
                topk = self.topk[active_adapter]
                dropout = self.vblora_dropout[active_adapter]
                x = x.to(vblora_vector_bank.dtype)
                # A: (rank, in_tile, vector_length) -> (rank, in_tile x vector_length)
                A = Linear._get_low_rank_matrix(vblora_logits_A, vblora_vector_bank, topk).reshape(
                    vblora_logits_A.shape[0], -1
                )
                # B: (out_tile, rank, vector_length) -> (out_tile, vector_length, rank) -> (out_tile x vector_length, rank)
                B = (
                    Linear._get_low_rank_matrix(vblora_logits_B, vblora_vector_bank, topk)
                    .transpose(1, 2)
                    .reshape(-1, vblora_logits_B.shape[1])
                )
