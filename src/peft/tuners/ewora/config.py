...
class LoftQConfig:
    """
    This is the sub-configuration class to store the configuration of a [`EworaModel`].
...
class EworaConfig(PeftConfig):
    ...
    Args:
        ...
        bias (`str`):
            Bias type for EWoRA. Can be 'none', 'all' or 'ewora_only'. If 'all' or 'ewora_only', the corresponding biases
        ...
        init_ewora_weights (`bool` | `Literal["gaussian", "olora", "pissa", "pissa_niter_[number of iters]", "loftq"]`):
            How to initialize the weights of the adapter layers. Passing True (default) results in the default
            initialization from the reference implementation from Microsoft. Passing 'gaussian' results in Gaussian
            initialization scaled by the EWoRA rank for linear and layers. Setting the initialization to False leads to
        ...
        megatron_config (`Optional[dict]`):
            ...
            parameter when you want to apply EWoRA to the ColumnParallelLinear and RowParallelLinear layers of megatron.
        megatron_core (`Optional[str]`):
            The core module from Megatron to use, defaults to `"megatron.core"`.
        loftq_config (`Optional[LoftQConfig]`):
            The configuration of LoftQ. If this is not None, then LoftQ will be used to quantize the backbone weights
            and initialize Ewora layers. Also pass `init_ewora_weights='loftq'`. Note that you should not pass a
    ...
    init_ewora_weights: bool | Literal["gaussian", "olora", "pissa", "pissa_niter_[number of iters]", "loftq"] = field(
        default=True,
        metadata={
            "help": (
                "How to initialize the weights of the EWoRA layers. Passing True (default) results in the default "
                "initialization from the reference implementation from Microsoft. Passing 'gaussian' results "
                "in Gaussian initialization scaled by the EWoRA rank for linear and layers. Setting the initialization "
    ...
    loftq_config: Union[LoftQConfig, dict] = field(
        ...
        metadata={
            "help": (
                ...
                "weights and initialize Ewora layers. Also set `init_ewora_weights='loftq'` in this case."
            )
        },
    )
    use_dora: bool = field(
    ...
    def __post_init__(self):
        ...
        # handle init_ewora_weights and loftq_config
        if self.init_ewora_weights == "loftq":
            import importlib

            if not importlib.util.find_spec("scipy"):
                raise ImportError("The required package 'scipy' is not installed. Please install it to continue.")
            if self.loftq_config is None:
                raise ValueError("`loftq_config` must be specified when `init_ewora_weights` is 'loftq'.")
