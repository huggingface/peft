...
class DeLoRAModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_target_module_exists(delora_config, key):
        return check_target_module_exists(delora_config, key)
    ...
    def _prepare_adapter_config(peft_config, model_config):
        if peft_config.target_modules is None:
            # Fallback to DeLoRA mapping for target modules
            if model_config["model_type"] not in TRANSFORMERS_MODELS_TO_DELORA_TARGET_MODULES_MAPPING:
                raise ValueError("Please specify `target_modules` in `peft_config`")
            peft_config.target_modules = set(
                TRANSFORMERS_MODELS_TO_DELORA_TARGET_MODULES_MAPPING[model_config["model_type"]]
            )
        return peft_config
    ...
    def merge_and_unload(
        ...
        r"""
        This method merges the DeLoRA layers into the base model. This is needed if someone wants to use the base model as
        a standalone model.
    ...
    def unload(self) -> torch.nn.Module:
        """
        Gets back the base model by removing all the DeLoRA modules without merging. This gives back the original base
        model.
        """
