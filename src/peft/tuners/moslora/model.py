...
from .layer import MoSLoraLayer, dispatch_default
...
class MoSLoraModel(BaseTuner):
    """
    Insert an extra mixer in the LoRA to mix information from different subspaces.

    The method is described in detail in https://arxiv.org/pdf/2406.11909.

    Args:
        model ([`torch.nn.Module`]): The model to be adapted.
        config ([`MoSLoraConfig`]): The configuration of the MoSLoraConfig model.
        adapter_name (`str`): The name of the adapter, defaults to `"default"`.
        low_cpu_mem_usage (`bool`, `optional`, defaults to `False`):
            Create empty adapter weights on meta device. Useful to speed up the loading process.
        use_moslora (`bool` `kai` `orth`): select strategy how to initilize mixer, `False mean the same as vanilla LoRA`
    ...
    Example:
        ...
        >>> from peft import MoSLoraModel, MoSLoraConfig
        ...
        >>> lora_model = MoSLoraModel(model, config, "default")
        ```

        ```py
        >>> import torch
        >>> import transformers
        >>> from peft import MoSLoraConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training

        >>> rank = ...
        >>> target_modules = ["q_proj", "k_proj", "v_proj", "out_proj", "fc_in", "fc_out", "wte"]
        >>> config = MoSLoraConfig(
        ...     r=4, use_moslora="orth", lora_alpha=16, target_modules=target_modules, lora_dropout=0.1, bias="none", task_type="CAUSAL_LM"
        ... )
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: MoSLoraConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: MoSLoraConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, MoSLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, MoSLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, MoSLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, MoSLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, MoSLoraLayer) or isinstance(module, ModulesToSaveWrapper):
    ...
    def _check_add_weighted_adapter(self, *args, **kwargs) -> None:
    ...
    def add_weighted_adapter(self, *args, **kwargs) -> None:
        """This method is not supported for AdaLoRA, use LoRA instead."""
        raise TypeError(f"{self.__class__.__name__} does not support add_weighted_adapter method.")

    def _svd_generalized_task_arithmetic_weighted_adapter(self, *args, **kwargs) -> None:
        """This method is not supported for AdaLoRA, use LoRA instead."""
        raise TypeError(f"{self.__class__.__name__} does not support add_weighted_adapter method.")

    def _generalized_task_arithmetic_weighted_adapter(self, *args, **kwargs) -> None:
        """This method is not supported for AdaLoRA, use LoRA instead."""
        raise TypeError(f"{self.__class__.__name__} does not support add_weighted_adapter method.")
    ...
    def delete_adapter(self, adapter_name: str) -> None:
        ...
        for key in key_list:
            ...
            if isinstance(target, MoSLoraLayer):
    ...
    def subtract_mutated_init(self, *args, **kwargs) -> None:
        """This method is not supported for AdaLoRA, use LoRA instead."""
        raise TypeError(f"{self.__class__.__name__} does not support add_weighted_adapter method.")
