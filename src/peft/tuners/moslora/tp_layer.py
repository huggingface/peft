...
class LoraParallelLinear(nn.Module, MoSLoraLayer):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        MoSLoraLayer.__init__(self, base_layer=base_layer, **kwargs)
    ...
    def update_layer(
        ...
        **parallel_linear_kwargs,
        ...
        # moslora init
        if isinstance(use_moslora, str):
            self.lora_mixer[adapter_name] = nn.Linear(r, r, bias=False, dtype=torch.float32) 
            self.use_moslora[adapter_name] = use_moslora
        elif use_moslora is True:
            self.lora_mixer[adapter_name] = nn.Linear(r, r, bias=False, dtype=torch.float32) 
