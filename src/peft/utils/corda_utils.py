...
def preprocess_corda(
    ...
    The fields are:
        ...
        eigens.S_cov (`torch.Tensor`):
            Singular values of the covariance matrix. Built only when `run_svd_for_covariance` is True.
        eigens.S_adjusted (`torch.Tensor`):
            Adjusted singular values of the weight matrix, in a way that norm of each column of `V_WC` is 1.
    ...
    if cache_file is not None and os.path.exists(cache_file) and os.path.getsize(cache_file) > 0:
        ...
        for name, module in target_modules(model, config):
            ...
            module.eigens = CordaEigens(
                ...
                S_adjusted=cache[f"{name}.eigens.S_adjusted"],
        ...
        if cache_file is not None:
            ...
            for name, module in target_modules(model, config):
                ...
                cache[f"{name}.eigens.S_adjusted"] = module.eigens.S_adjusted
    ...
    for name, module in target_modules(model, config):
        config.rank_pattern[name] = int(module.rank)
        config.alpha_pattern[name] = int(module.rank * ratio)
...
def collect_eigens_for_layer(
    ...
    run_svd_for_covariance = config.corda_config.get("run_svd_for_covariance")
    ...
    logging.info(f"S_adjusted: {S_adjusted[:16]} ... {S_adjusted[-16:]}")
