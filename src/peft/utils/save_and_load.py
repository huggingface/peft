...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        if platform.system() == "Windows":
            warnings.warn(
                "Windows has issues saving integers into safetensors. Hence, we convert shira_indices to float32 before saving on Windows OS. The shira_indices will always be converted to integers when loading."
            )
        for name, module in model.named_modules():
            if hasattr(module, "shira_indices"):
                for k, v in module.shira_indices.items():
                    # Windows has some issues with saving integers into safetensors. Tests fail with some kind of PermissionError.
                    # This results in failed tests, so we are converting indices to float32 before saving and then converting them back to int when loading.
                    # This is happening only for Windows, not for Linux and Mac-OS.
...
def set_peft_model_state_dict(
    ...
    if config.is_prompt_learning or config.peft_type == PeftType.ADAPTION_PROMPT:
        ...
        if config.peft_type == PeftType.ADALORA:
            ...
            if platform.system() == "Windows":
                warnings.warn(
                    "Windows has issues saving integers into safetensors. Hence, we had converted shira_indices to float32 before saving on Windows OS. The shira_indices will always be converted to integers when loading."
                )
            for name, module in model.named_modules():
                if hasattr(module, "shira_indices"):
                    # for k, v in module.shira_indices.items():
                    if f"{name}.shira_indices.{adapter_name}" in peft_model_state_dict:
                        ...
                        # Convert shira_indices to int in case they were saved on a Windows OS and are being loaded on a Linux or a Mac-OS system.
                        # If they were saved in Linux or Mac-OS, they are already integers and the following will not affect anything.
