...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_r: int = field(
        default=None,
        metadata={"help": "The rank of LoRA adapter. When passing `None`, Astra or full fine-tuning is used."},
    )
    astra_mode: bool = field(default=True, metadata={"help": "True for Astra mode"})
...
def train():
    ...
    if script_args.astra_mode:
        print("Train in Astra mode")
        res_model = transformers.AutoModelForCausalLM.from_pretrained(
            script_args.model_name_or_path,
            device_map="auto",
        )
        model = PeftModel.from_pretrained(
            res_model, script_args.model_name_or_path, subfolder="astra_init", is_trainable=True
        )
