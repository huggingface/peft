...
def parse_args(input_args=None):
    ...
    # deft args
    parser.add_argument("--use_deft", action="store_true", help="Whether to use DEFT for parameter efficient tuning")
    parser.add_argument("--deft_r", type=int, default=8, help="DEFT rank, only used if use_deft is True")
    parser.add_argument(
        "--deft_alpha", type=int, default=16, help="DEFT injection scaling (alpha / r), only used if use_deft is True"
    )
    parser.add_argument(
        ...
        default="qr",
        choices=["relu", "qr"],
        help="How the DEFT projector is derived from P: 'relu' or 'qr' (default, best for image generation)",
    )
    parser.add_argument("--deft_dropout", type=float, default=0.0, help="DEFT dropout, only used if use_deft is True")

    parser.add_argument(
        "--deft_text_encoder_r",
        type=int,
        default=8,
        help="DEFT rank for text encoder, only used if `use_deft` and `train_text_encoder` are True",
    )
    parser.add_argument(
        "--deft_text_encoder_alpha",
        type=int,
        default=16,
        help="DEFT injection scaling for text encoder, only used if `use_deft` and `train_text_encoder` are True",
    )
    parser.add_argument(
        "--deft_text_encoder_decomposition_method",
        type=str,
        default="qr",
        choices=["relu", "qr"],
        help="DEFT decomposition method for the text encoder, only used if `use_deft` and `train_text_encoder` are True",
    )
    parser.add_argument(
        "--deft_text_encoder_dropout",
        type=float,
        default=0.0,
        help="DEFT dropout for text encoder, only used if `use_deft` and `train_text_encoder` are True",
    )
...
def main(args):
    ...
    if args.use_deft:
        config = DeftConfig(
            r=args.deft_r,
            alpha=args.deft_alpha,
            decomposition_method=args.deft_decomposition_method,
    ...
    elif args.train_text_encoder and args.use_deft:
        config = DeftConfig(
            r=args.deft_text_encoder_r,
            alpha=args.deft_text_encoder_alpha,
            decomposition_method=args.deft_text_encoder_decomposition_method,
            target_modules=TEXT_ENCODER_TARGET_MODULES,
            deft_dropout=args.deft_text_encoder_dropout,
            init_weights=True,
        )
    ...
    if args.gradient_checkpointing:
        ...
        # below fails when using deft so commenting it out
        if args.train_text_encoder and not args.use_deft:
