#!/bin/bash
usage() {
  echo "Usage: ${0} [-t|--type] [-d|--dataset] [-n|--block_num] [-s|--block_size] [-f|--n_butterfly_factor]"  1>&2
  exit 1
}
...
if [ $MODEL_NAME = "stabilityai/stable-diffusion-2-1" ]; then
  export RUN_NAME="${RUN_NAME}_stable"
elif [ $MODEL_NAME = "runwayml/stable-diffusion-v1-5" ]; then
  export RUN_NAME="${RUN_NAME}_runway"
else
...
if [[ $PEFT_TYPE =~ ^(boft|oft)$ ]]; then
  ...
  accelerate launch --main_process_port=29505 train_controlnet_tmp.py \
  ...
  accelerate launch --main_process_port=29505 train_controlnet.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --resume_from_checkpoint=$RESUME_PATH \
  --controlnet_model_name_or_path=$CONTROLNET_PATH \
  --output_dir=$OUTPUT_DIR \
